...
function getLibrary(key){
    ...
    if (data) {
        data.forEach((instObj) => {
            instObj.__proto__ = Institution.prototype;
        });
    }
    return data;
}

function updateChanges(storageKey, identifier){
    saveData(storageKey);
    clearTable(identifier);
    render();
}

function addToLibrary(myLibrary, data, storageKey) {
    myLibrary.push(data);
    updateChanges(storageKey);
}

function removeFromLibrary(myLibrary, id, storageKey, identifier){
	myLibrary.splice(id, 1);
    updateChanges(storageKey, identifier);
}
